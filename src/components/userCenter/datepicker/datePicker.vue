...
2018-3-13 -->
  <div class="am-calendar">
    <transition name="am-calendar-mask">
      <div class="am-calendar-backdrop"
           @click="closeCalendar"
           v-show="showCalendar"></div>
    </transition>
    <transition name="am-calendar-content">
      <div class="am-calendar-content am-calendar-content-transition"
           v-show="showCalendar"
           ref="amCalendar">
        <div class="am-calendar-title">
          ...
          <span class="butn_confirm"
                @click="successCallback">确认</span>
        </div>
        ...
        <div class="month-bar-fixed"
             id="fixedBarEle"
             ref="fixedBar"
             :style="{'opacity': fixedBarOpacity}">{{ fixedMonthbar }}</div>
    ...
    <no-select-time :toastFlat='toastFlat'></no-select-time>
  </div>
</template>
...
import NoSelectTime from './noSelectTime'
  ...
  methods: {
    ...
    successCallback () {
      ...
      let end = this.isEndDate
        ...
        : start
      if (!start || !end) {
        _that.$emit('changeDate', start, end)
        _that.toastFlat = true
        setTimeout(() => {
          _that.toastFlat = false
        }, 1000)
        return
      }
...
.am-calendar-title .butn_confirm {
  font-size: 16px;
  margin-right: 15px;
}
